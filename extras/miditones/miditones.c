...
*      Generate "const" for data initialization for compatibility with Arduino
IDE v1.6.x.
...
*
*
*  MIDITONES converts a MIDI music file into a much simplified stream of
commands,
...
*
*  Volume ("velocity") and instrument specifications in the MIDI files are
discarded.
*  All the tracks are prcoessed and merged into a single time-ordered stream of
*  "note on", "note off", and "delay" commands.
*
*  This was written for the "Playtune" Arduino library, which plays polyphonic
music
*  using up to 6 tone generators run by the timers on the processor.  See the
separate
*  documentation for Playtune.  But MIDITONES may prove useful for other tone
*  generating systems.
*
*  The output can be either a C-language source code fragment that initializes
an
*  array with the command bytestream, or a binary file with the bytestream
itself.
*
*  MIDITONES is written in standard ANSI C (plus strlcpy and strlcat functions),
and
...
*  wide variety of file types.  In particular, we have tested only format type
"1",
...
*  This has been tested only on a little-endian PC, but I think it should work
on
...
*  the C-language statement to intiialize an array called "score" with the
bytestream.
*
*
...
*
*  The input file is the base name with the extension ".mid".  The output
filename(s)
...
*       Tracks are processed sequentially instead of being merged into
chronological order.
*       This is mostly useful when generating a log to debug MIDI file parsing
problems.
*
*  -lp  Log input file parsing information to the <basefilename>.log file
*
*  -lg  Log output bytestream generation information to the <basefilename>.log
file
*
...
*       exist on your processor because any extra notes will be ignored,
although
*       it does make the file bigger than necessary . Of course, too many
ignored
...
*
*  If the high-order bit of the byte is 1, then it is one of the following
commands:
*
*    9t nn [vv] Start playing note nn on tone generator t.  Generators are
numbered
*           starting with 0.  The notes numbers are the MIDI numbers for the
chromatic
*           scale, with decimal 60 being Middle C, and decimal 69 being Middle A
(440 Hz).
*           if the -v option is enabled, a second byte is added to indicate
velocity
*
...
*
*  If the high-order bit of the byte is 0, it is a command to delay for a while
until
*  the next note change..  The other 7 bits and the 8 bits of the following byte
are
*  interpreted as a 15-bit big-endian integer that is the number of milliseconds
to
*  wait before processing the next command.  For example,
*
*     07 D0
*
*  would cause a delay of 0x07d0 = 2000 decimal millisconds, or 2 seconds.  Any
tones
...
#define MAX_TONEGENS \
  16 /* max tone generators: tones we can play simultaneously */
...
struct track_status {             /* current processing point of a MIDI track */
  ...
  bool tonegens
      [MAX_TONEGENS]; /* which tone generators our notes are playing on */
} track[MAX_TRACKS] = {{0}};
...
/* if CMD < 0x80, then the other 7 bits and the next byte are a 15-bit number of
 * msec to delay */
    ...
    if (argv[i][0] == '/' || argv[i][0] == '-') {
      switch (toupper(argv[i][1])) {
        ...
        case 'C':
          ...
          break;
        case 'K':
          if (sscanf(&argv[i][2], "%d", &keyshift) != 1 || keyshift < -100 ||
              keyshift > 100)
            goto opterror;
...
void process_header(void) {
  ...
  if (time_division < 0x8000)
    ...
    ticks_per_beat = ((time_division >> 8) & 0x7f) /* SMTE frames/sec */ *
                     (time_division & 0xff); /* ticks/SMTE frame */
  ...
  hdrptr += rev_long(hdr->header_size) +
            8; /* point past header to track header, presumably. */
  return;
}
  ...
  while (t->trkptr < t->trkend) {
    ...
    if (event == 0xff) { /* meta-event */
      ...
      switch (meta_cmd) {
        ...
        default: /* assume it is a string */
          ...
          if (tracknum == 0 && meta_cmd == 0x03 && !parseonly &&
              !binaryoutput) {
            /* Incredibly, MIDI has no standard for recording the name of the
            piece!
      ...
      switch (event >> 4) {
        ...
        note_off:
          if (logparse)
            fprintf(logfile, "note %02X off, chan %d, velocity %d\n", t->note,
                    chan, velocity);
        ...
        case 0x9:
          ...
          velocity = *t->trkptr++;
          if (velocity ==
              0) /* some scores use note-on with zero velocity for off! */
            goto note_off;
          t->velocity = velocity;
          if (logparse)
            fprintf(logfile, "note %02X on,  chan %d, velocity %d\n", t->note,
                    chan, velocity);
          if ((1 << chan) & channel_mask) {  // if we're processing this channel
            t->cmd = CMD_PLAYNOTE;
            return; /* stop processing and return */
...
#define MAXPATH 120
  ...
  for (tracknum = 0; tracknum < num_tracks; ++tracknum) {
    ...
    if (parseonly)
      while (track[tracknum].cmd != CMD_TRACKDONE) find_note(tracknum);
  }

  /* Continue processing all tracks, in an order based on the simulated time.
  This is not unlike multiway merging used for tape sorting algoritms in the
  50's! */
  ...
  if (!parseonly) do { /* while there are still track notes to process */
      ...
      */
      ...
      /* Usually we start with the track after the one we did last time
      (tracknum),
      so that if we run out of tone generators, we have been fair to all the
      tracks.
      The alternate "strategy1" says we always start with track 0, which means
      that we favor early tracks over later ones when there aren't enough tone
      generators.
      */
      ...
      /*	If this track event is "set tempo", just change the global
      tempo.
      ...
      /*	If this track event is "stop note", process it and all
      subsequent "stop notes" for this track
      that are happening at the same time. Doing so frees up as many tone
      generators as possible.  */

      else if (trk->cmd == CMD_STOPNOTE)
        do {
          // stop a note
          for (tgnum = 0; tgnum < num_tonegens;
               ++tgnum) { /* find which generator is playing it */
            tg = &tonegen[tgnum];
            if (tg->playing && tg->track == tracknum && tg->note == trk->note) {
              if (loggen)
                fprintf(logfile, "->Stop note %02X, generator %d, track %d\n",
      ...
      Don't do more than one, so we allow other tracks their chance at grabbing
      tone generators. */

      else if (trk->cmd == CMD_PLAYNOTE) {
        bool foundgen = false;

        if (strategy2) { /* try to use the same tone generator this track used
                            last time */
        ...
        if (!foundgen)
          for (tgnum = 0; tgnum < num_tonegens;
               ++tgnum) { /* search for a free tone generator */
            tg = &tonegen[tgnum];
            if (!tg->playing) {
              foundgen = true;
              break;
        ...
        if (foundgen) {
          ...
          if (loggen)
            fprintf(logfile, "->Start note %02X, generator %d, track %d\n",
          ...
          if (binaryoutput) {
            ...
            if (velocityoutput == 0) {
              fprintf(outfile, "0x%02X,%d, ", CMD_PLAYNOTE | tgnum,
                      shifted_note);
              outfile_items(2);
            } else {
              fprintf(outfile, "0x%02X,%d,%d, ", CMD_PLAYNOTE | tgnum,
                      shifted_note, trk->velocity);
              outfile_items(3);
            }
          }
        } else {
  ...
  if (!parseonly) {
    ...
    if (binaryoutput)
      ...
      fprintf(outfile,
              "0x%02x};\n// This score contains %ld bytes, and %d tone "
              "generator%s used.\n",
    ...
    if (notes_skipped)
      printf(
          "  %d notes were skipped because there weren't enough tone "
          "generators.\n",
          notes_skipped);
