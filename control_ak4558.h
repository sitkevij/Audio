...
// if 1, then Serial Monitor will show debug information about configuration of
// the AK4558
...
// PMPLL = 0 (EXT Slave Mode; disables internal PLL and uses ext. clock) (by
// DEFAULT)
// ACKS = 0 (Manual Setting Mode; disables automatic clock selection) (by
// DEFAULT)
// DFS1-0 = 00 (Sampling Speed = Normal Speed Mode, default)
// MCKS1-0 = 00 (Master Clock Input Frequency Select, set 256fs for Normal Speed
// Mode -> 11.2896 MHz)
...
#endif
// Chip Address 1 pin
// set to 'H' by default, configurable to 'L' via a jumper on bottom side of the
// board
...
// datasheet page 81:
// This address is 7 bits long followed by the eighth bit that is a data
// direction bit (R/W).
// The most significant five bits of the slave address are fixed as “00100”. The
// next bits are
// CAD1 and CAD0 (device address bit). These bits identify the specific device
// on the bus.
// The hard-wired input pins (CAD1 and CAD0) set these device address bits
// (Figure 69)
...
// When ACKS bit = “1”, master clock frequency is detected automatically. In
// this case, the setting of
// DFS1-0 bits is ignored. When ACKS bit = “0”, DFS1-0 bits set the sampling
// speed mode. The MCKI
...
class AudioControlAK4558 : public AudioControl {
 public:
  bool enable(void);  // enables the CODEC, does not power up ADC nor DAC (use
                      // enableIn() and enableOut() for selective power up)
  bool enableIn(void);  // powers up ADC
  bool enableOut(void);  // powers up DAC
  bool disable(void) {
    return (disableIn() && disableOut());
  }  // powers down ADC/DAC
  ...
  bool inputSelect(int n) {
    return false;
  }  // sets inputs to mono left, mono right, stereo (default stereo), not yet
     // implemented
 private:
