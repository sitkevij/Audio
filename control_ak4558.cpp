...
// ADC setup (datasheet page 74
  ...
  // ignore this, leaving default values - ADC: Set up the de-emphasis filter
  // (Addr = 07H).
...
// Initialization cycle of the ADC is 5200/fs @Normal mode. The SDTO pin outputs
...
bool AudioControlAK4558::enableOut(void) {
  ...
  // ignore this, leaving default values - Set up the digital output volume
  // (Address = 08H, 09H).
  ...
  // Outputs of the LOUT and ROUT pins start rising. Rise time is 300ms (max.)
  // when C = 1μF.
...
// Set LOPS bit to “0” after the LOUT and ROUT pins output “H”. Sound data will
// be output from the
...
#endif
  // Control register settings become available in 10ms (min.) when LDOE pin =
  // “H”
  Wire.begin();
  initConfig();
  // access all registers to store locally their default values

  // DIF2-0, DFS1-0 and ACKS bits must be set before MCKI, LRCK and BICK are
  // supplied

  // PMPLL = 0 (EXT Slave Mode; disables internal PLL and uses ext. clock)
  // (by DEFAULT)
  // ACKS = 0 (Manual Setting Mode; disables automatic clock selection)
  // (by DEFAULT)
  // DFS1-0 = 00 (Sampling Speed = Normal Speed Mode)							(by
  // DEFAULT)
  // TDM1-0 = 00 (Time Division Multiplexing mode OFF) 						(by
  // DEFAULT)
...
#endif
  // MCKS1-0 = 00 (Master Clock Input Frequency Select, set 256fs for Normal
  // Speed Mode -> 11.2896 MHz)
...
// Set up the sampling frequency (FS3-0 bits). The ADC must be powered-up in
// consideration of PLL
...
// Write configuration registers in a single write operation (datasheet page
// 81):
// The AK4558 can perform more than one byte write operation per sequence. After
// receipt of the third byte
// the AK4558 generates an acknowledge and awaits the next data. The master can
// transmit more than
// one byte instead of terminating the write cycle after the first data byte is
// transferred. After receiving each
// data packet the internal address counter is incremented by one, and the next
// data is automatically taken
...
bool AudioControlAK4558::disableOut(void) {
  ...
  // Outputs of the LOUT and ROUT pins start falling. Rise time is 300ms (max.)
  // when C = 1μF.
