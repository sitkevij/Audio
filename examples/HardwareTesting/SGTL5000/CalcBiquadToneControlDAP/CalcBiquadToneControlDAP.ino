/* Tone example using SGTL5000 DAP PEQ filters and calcBiquad filter calculator
routine.
...
void setup() {
  ...
  calcBiquad(FILTER_PARAEQ, 110, 0, 0.2, 524288, 44100,
             updateFilter);  // automation negates the need
  audioShield.eqFilter(
      0, updateFilter);  // for the three lines commented out above.
...
void loop() {
  ...
  if (chgMsec > 10) {
    ...
    if (tone2 != tone1) {
      ...
      calcBiquad(FILTER_PARAEQ, 4400, tone2, 0.167, 524288, 44100,
                 updateFilter);
      audioShield.eqFilter(1, updateFilter);
      tone1 = tone2;
    }
