...
Bounce button1 =
    Bounce(1, 5);  // cycles the bitcrusher through some key samplerates
Bounce button2 =
    Bounce(2, 5);  // turns on the compressor (or "Automatic Volume Leveling")
...
void setup() {
  ...
  // Bitcrusher
  left_BitCrusher.bits(current_CrushBits);  // set the crusher to defaults. This
                                            // will passthrough clean at
                                            // 16,44100
  left_BitCrusher.sampleRate(current_SampleRate);  // set the crusher to
                                                   // defaults. This will
                                                   // passthrough clean at
                                                   // 16,44100
  right_BitCrusher.bits(current_CrushBits);  // set the crusher to defaults.
                                             // This will passthrough clean at
                                             // 16,44100
  right_BitCrusher.sampleRate(current_SampleRate);  // set the crusher to
                                                    // defaults. This will
                                                    // passthrough clean at
                                                    // 16,44100
                                                    // Bitcrusher
...
*/
  ...
  audioShield.autoVolumeControl(
      1, 1, 0, -6, 40, 20);  // **BUG** with a max gain of 0, turning the AVC
                             // off leaves a hung AVC problem where the attack
                             // seems to hang in a loop. with it set 1 or 2,
                             // this does not occur.
...
void loop() {
  ...
  if (!(playWav1.isPlaying())) {
    playWav1.play(
        "SDTEST1.WAV");  // http://www.pjrc.com/teensy/td_libs_AudioDataFiles.html
  }

  if (button0.fallingEdge()) {
    // Bitcrusher BitDepth
    if (current_CrushBits >=
        2) {  // eachtime you press it, deduct 1 bit from the settings.
      current_CrushBits--;
    } else {
  ...
  if (button1.fallingEdge()) {
    // Bitcrusher SampleRate // the lowest sensible setting is 345. There is a
    // 128 sample buffer, and this will copy sample 1, to each of the other 127
    // samples.
    if (current_SampleRate >= 690) {  // 345 * 2, so we can do one more divide
      current_SampleRate =
          current_SampleRate / 2;  // half the sample rate each time
    } else {
      current_SampleRate = 44100;  // if you get down to the minimum then go
